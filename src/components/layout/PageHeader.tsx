...
export function PageHeader({ showRegisterLink = false, maxWidth = "3xl" }: PageHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-amber-200/30 bg-white/70 backdrop-blur-md">
      <div className={`mx-auto flex items-center justify-between px-4 py-4 sm:px-6 ${widthClasses}`}>
        ...
        <Link href="/" className="group flex items-center gap-2 transition-transform duration-250 active:scale-[0.98]">
          <span className="text-xl font-black tracking-wider bg-gradient-to-r from-amber-700 via-amber-900 to-amber-950 bg-clip-text text-transparent sm:text-2xl transition-all duration-300 group-hover:opacity-85">
            ስምህ ይቀደስ
          </span>
        </Link>
        ...
        <div className="flex items-center gap-5">
          ...
          <a
            ...
            className="text-amber-900/60 hover:text-amber-900 transition-colors duration-200"
            title="Telegram"
          >
        ...
        <div className="flex items-center gap-4">
          {showRegisterLink && (
            <Link
              href="/register"
              className="relative inline-flex items-center justify-center overflow-hidden rounded-xl bg-gradient-to-r from-[#dec084] via-[#eedbb3] to-[#cba358] text-[#3c2f17] border border-[#cba358]/20 px-5 py-2 text-sm font-bold shadow-[0_4px_12px_rgba(222,192,132,0.2)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.97]"
            >
              ምዝገባ
            </Link>
          )}
